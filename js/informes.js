const mensajeAmarillo = document.getElementById("mensaje-amarillo-informes");
const textoAmarillo = document.getElementById("texto-amarillo-informes");
const mensajeVerde = document.getElementById("mensaje-verde-informes");
const textoVerde = document.getElementById("texto-verde-informes");
const mensajeRojo = document.getElementById("mensaje-rojo-informes");
const textoRojo = document.getElementById("texto-rojo-informes");
const footer = document.getElementById("fijar-footer"); //div que contiene el footer
const dataLocalStorage = localStorage.getItem('INFORMES');
const tablaCompleta = document.getElementById('sec-contenido');
const cuadro = document.getElementById('recuadro-completo');
...
function verificarLocal() {
    //Verifica si la cadena existe y la divide en un array llamado arrayDatos usando la coma como separador. Si la cadena no existe, se asigna un array vacío 
    let arrayDatos = dataLocalStorage ? dataLocalStorage.split(',') : [];
    // Verificar si hay elementos en arrayDatos
    if (arrayDatos.length > 0) {
        for (let i = 0; i < arrayDatos.length; i++) {

            const registro = arrayDatos[i];
            const datosSeparados = registro.split('|');

            const añoElegido = datosSeparados[0];
            const tipoRecuento = datosSeparados[1];
            const tipoEleccion = datosSeparados[2];
            const categoriaId = datosSeparados[3];
            const distritoElegidoId = datosSeparados[4];
            const seccionProvinciaElegida = datosSeparados[5];
            const idSeccionElegidaId = datosSeparados[6];
            const circuitoId = datosSeparados[7];
            const mesaId = datosSeparados[8];
            const cargoElegido = datosSeparados[9];
            const distritoElegido = datosSeparados[10];
            const seccionElegida = datosSeparados[11];
            ...
            llamarAPI(url)
                .then(dataFiltrar => {
                    ...
                    let contenedorPrincipal = fila.querySelector(".datos-agrupacion");
                    dataFiltrar.valoresTotalizadosPositivos.forEach(agrupacion => {
                        contenedorPrincipal.innerHTML += 
                        `<div class="nombre-agrupacion">
                            <p class="nombre-agrupacion-p">${agrupacion.nombreAgrupacion}</p>
                            <div class="porcentaje-votos">
                                <p class="porcentaje-votos-p">${agrupacion.votosPorcentaje}</p>
                                <p class="numero-votos">${agrupacion.votos}</p>
                            </div>
                        </div>`
                    });
