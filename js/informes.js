...
let dataLocalStorage = localStorage.getItem('INFORMES');
let tablaCompleta = document.getElementById('sec-contenido');
let estadoRecuento = {}; //inicializarlo en un objeto vacio
...
async function verificarLocal() {
    ...
    let arrayDatos = dataLocalStorage ? dataLocalStorage.split(',') : []; //Verifica si la cadena existe y la divide en un array llamado arrayDatos usando la coma como separador. Si la cadena no existe, se asigna un array vacío
        ...
        for (let i = 0; i < arrayDatos.length; i++) {
            ...
            fetch(`https://elecciones-lc2.bruselario.com/api/resultados/getResultados/?anioEleccion=${añoElegido}&tipoRecuento=${tipoRecuento}&tipoEleccion=${tipoEleccion}&categoriaId=${categoriaId}&distritoId=${distritoElegidoId}&seccionProvincialId=${seccionProvinciaElegida}&seccionId=${idSeccionElegidaId}&circuitoId=${circuitoId}&mesaId=${mesaId}`)
                ...
                .then(data => {
                    ...
                    fila.innerHTML += `
                    <td class="cvg">
                        <div class="svg-provincias"></div>
                    </td>
                    <td class="th-grilla-texto">
                        <div class="grilla-texto">
                            <p class="texto-elecciones-chico" p>
                            <p class="texto-path-chico"></p>
                        </div>
                    </td>
                    <td class="th-datos-generales">
                        <div class="datos-generales">
                            <div class="boton-mesas-escrutadas">
                                <div class="svg-mesas-escrutadas"><svg version="1.1" id="Capa_1"
                                        ...
                                        style="font-size: 50px; height: 50px; width: 50px; margin: 10px 0px 10px 4px;">
                                        <g>
                                            <path fill="#FFFFFF"
                                ...
                                <div class="texto-porcentaje">
                                        <p class="texto-mesas-escrutadas">Mesas Escrutadas</p>
                                        <p class="porcentaje-mesas-escrutadas"></p>
                                </div>
                            </div>
                            <div class="boton-electores">
                                <div class="svg-electores"><svg version="1.1" id="Layer_1"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                        width="841.889px" height="595.281px" viewBox="0 0 841.889 595.281"
                                        enable-background="new 0 0 841.889 595.281" xml:space="preserve"
                                        class="v-icon__component theme--light"
                                        style="font-size: 50px; height: 50px; width: 50px; margin-top:8px;">
                                ...
                                <div class="texto-porcentaje">
                                    <p class="texto-electores">Electores</p>
                                    <p class="porcentaje-electores"></p>
                                </div>
                            </div>
                            <div class="boton-participacion-escrutado">
                                <div class="svg-part-escrutado"><svg version="1.1" id="Layer_1"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                        width="841.889px" height="595.281px" viewBox="0 0 841.889 595.281"
                                        enable-background="new 0 0 841.889 595.281" xml:space="preserve"
                                ...
                                <div class="texto-porcentaje">
                                    <p class="texto-part-escrutado">Participación <br>sobre escrutado</p>
                                    <p class="porcentaje-part-escrutado"></p>
                                </div>
                            </div>
                        </div>
                    </td>
                    <td>
                        <div class="datos-agrupacion">
                            <div class="nombre-agrupacion">
                                <p class="nombre-agrupacion-p"></p>
                                <div class="porcentaje-votos">
                                    <p class="porcentaje-votos-p"></p>
                                    <p class="numero-votos"></p>
                                </div>
                            </div>
                        </div>
                    </td>
                    ...
                    //mapa
                    let svgDistrito = fila.querySelector('.svg-provincias');
                    svgDistrito.innerHTML = mapas[distritoElegido];

                    // titulo y subtitulo
                    let titulo = fila.querySelector(".texto-elecciones-chico");
                    let subtitulo = fila.querySelector(".texto-path-chico");
                    if (tipoEleccion == "2") {
                    ...
                    let mesasEscrutadas = fila.querySelector(".porcentaje-mesas-escrutadas");
                    mesasEscrutadas.innerHTML = dataFiltrar.estadoRecuento.mesasTotalizadas;
                    let electores = fila.querySelector(".porcentaje-electores");
                    electores.innerHTML = dataFiltrar.estadoRecuento.cantidadElectores;
                    let participacion = fila.querySelector(".porcentaje-part-escrutado");
                    participacion.innerHTML = dataFiltrar.estadoRecuento.participacionPorcentaje + "%";

                    //datos por agrupación
                    let contenedorPrincipal = document.querySelector(".datos-agrupacion");
                    dataFiltrar.valoresTotalizadosPositivos.forEach(agrupacion => {
                        let nuevoDivAgrupacion = document.querySelector(".nombre-agrupacion").cloneNode(true);
                        // Busca los elementos dentro del nuevo div clonado y asigna la información
                        let nombreAgrupacionElement = nuevoDivAgrupacion.querySelector(".nombre-agrupacion-p");
                        let porcentajeVotosElement = nuevoDivAgrupacion.querySelector(".porcentaje-votos-p");
                        let votosElement = nuevoDivAgrupacion.querySelector(".numero-votos");

                        // Asigna los valores a los elementos
                        nombreAgrupacionElement.innerHTML = agrupacion.nombreAgrupacion;
                        porcentajeVotosElement.innerHTML = agrupacion.votosPorcentaje;
                        votosElement.innerHTML = agrupacion.votos;

                        // Agrega el nuevo div al contenedor principal
                        contenedorPrincipal.appendChild(nuevoDivAgrupacion);
                    });
                })
                .catch (error => {
